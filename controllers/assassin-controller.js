...
exports.getAssassinProfile = async (req, res) => {
  try {
    const assassin = await User.aggregate([
      {
        $match: {
    ...
    res.status(200).json(assassin[0]);
  }
  catch (error) {
    console.error(error);
    res.status(500).json({ message: "Error al obtener el perfil del asesino" });
  }
}

exports.updateAssassinProfile = async (req, res) => {
  try {
    const schema = z.object({
      country: z.string({ required_error: "El país es obligatorio" }),
      address: z.string({ required_error: "La dirección es obligatoria" }),
